...
```bash
┌──(armando㉿kali)-[~/picoCTF/forensic/enhance]
└─$ ls
drawing.flag.svg
...
```
- Y al examinarlo notamos que en las líneas que tienen la etiqueta "</tspan>" contienen un texto peculiar por lo que le aplicamos un grep.
```bash
┌──(armando㉿kali)-[~/picoCTF/forensic/enhance]
...
└─$ 
```
- Y al hacerlo nos damos cuenta que ahí esta la bandera.
```bash
