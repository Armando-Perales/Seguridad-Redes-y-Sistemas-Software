...
## Solución
- Instalamos la librería `gcc-multilib` para ayudarnos con el código
```bash
┌──(armando㉿kali)-[~/picoCTF/reversing/asm4]
...
asm4:
        ...
        <+157>: ret    
```
- Le eliminamos la primera columna para que después podamos ejecutar el programa.
```
┌──(armando㉿kali)-[~/picoCTF/reversing/asm4]
...
└─$ nano chal.s
```
- Modificamos el código para que nos quede de la siguiente manera.
```bash
...
└─$ nano solve.c
```
- Creamos un código en C para que nos muestre la bandera.
```c
#include <stdio.h>
int main(){
        printf("Flag: 0x%x\n", asm4("picoCTF_724a2"));
