...
## Solución
- Inspeccionamos la pagina y vemos que en la hoja css manda a llamar a una imagen, así que la descargamos y le aplicamos un exiftool para observar sus propiedades.
```bash
┌──(armando㉿kali)-[~/picoCTF/forensic/milkslap]
...
Megapixels                      : 60.8
```
- Escribimos el siguiente comando para decirle a Ruby que el tamño de memoria sea mas grande y asi cargue la imagen.
```bash
┌──(armando㉿kali)-[~/picoCTF/forensic/milkslap]
